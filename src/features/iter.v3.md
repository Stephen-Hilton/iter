...
Authentication (decided 2026-09-01): baked-in JWT, not Cognito.  iter_data holds the auth: `/auth/login` exchanges username+password (argon2id "pwhash") for a signed JWT (~24h for humans); engines get long-lived tokens (~1y, "role": "engine") minted by an admin.  Verification is stateless signature-checking (secret/keypair in iter_data env), so it works identically in a container and on Lambda.  Revocation: a "tokenver" integer stamped into each token — bump it on the user row to invalidate everything outstanding for that principal.  "role" is one of user | engine | admin | viewer (added 2026-09-04: a viewer reads everything and gets 403 on every queue write — items, details, ELI5, schedules, reopen — but may edit their own profile: timezone, password; the webui hides its write controls for that role).  Cognito remains a later option if SSO/MFA is ever wanted.
...
### Cluster-restart block (built 2026-09-09)

Plan: `iter3/plans/cluster_restart_block.buildplan.md` (serves PDY-TECH-077 as amended 2026-09-09 and PDY-TECH-086).  The statement it makes true: *if you require the cluster for your work and it is between 02:00 and 06:00 PT, tag yourself as "blocked by: cluster restart" and exit without incrementing your attempt; the engine blocks your work item from restarting until the cluster is back up and healthy; once it is, you are no longer blocked; when your work item goes from queued to in-progress the tag is stripped.*

**Two tags, on purpose.**  The durable tag an agent (or a human) sets is `blocked-by-cluster-restart` — outside the engine-owned `blocked by: ` prefix, so `reconcile_waits` never deletes it.  The displayed tag `blocked by: cluster restart` is DERIVED by the engine every tick from the durable tag plus the cluster's health, exactly like every other wait reason above, and comes off the moment the reason clears.  Both are stripped by the claim that moves the item to in-progress, through the one function `iter_core::claim_tags` that the dispatch claim and the session-chaining claim both call (PDY-TECH-046).  Session chaining never takes a tagged neighbour: dispatch alone knows the cluster's health.

**The verb: `iter block --cluster-restart [--reason <text>]`** (sibling of `iter reject`).  One versioned PUT on the calling item: the durable tag if absent, `state: parked`, `lasterror: "blocked by: cluster restart — <reason>"` (the one field the next run reads back, in the prompt's "Previous attempt" section), and `attempt` decremented by one, floored at 0 — the claim incremented it before the agent ran a single turn, so putting it back is the whole of "exit without incrementing".  A `doc` row records the block and the counter's before/after.  The engine sees the mid-run state change at the turn boundary and keeps `parked` (no close gate, locks released), as it does for `reject`.

**The dispatch rule** (`engine.rs` dispatch, per item — not a project hold).  A queued item carrying the durable tag is never picked while the cluster is unavailable (the Run Now override included), and its wait reason is `cluster restart`.  A PARKED item carrying the tag shows the same reason while the cluster is unavailable and is REQUEUED by the engine — tag kept, `retry_after` cleared, a `doc` row naming the release — the tick the cluster is back up and healthy — unless the item is still in flight (running on this engine, or still holding a live central lock row: `iter block` parks the item mid-run, and requeueing before its turn ends would let the run close through the gate, seen in e2e 2026-09-09); the claim then strips the tag, so the row stays truthful right up to the moment the agent starts.  Health is evaluated once per tick per project, and only when some open item carries the tag (one GET of the newest window clone's detail rows).

**"Back up and healthy", concretely** (`iter_core::cluster::evaluate`, from `project.cluster_restart = {schedule, tz, from, until}`; defaults `America/Los_Angeles`, `02:00`–`06:00`, no schedule):
1. The newest work item whose `source_schedule` is the configured restart template decides when it ran within the last 24 h: `complete` with a `clusterhealth` detail row whose `bringup_exit` and `status_exit` are both 0 = healthy; `complete` with a red row = unavailable (a red night stays blocked until a green window or 24 h pass — the alternative is dispatching agents into a cluster the status check called broken); still open (the window is running now) = unavailable; `failed` = unavailable.
2. Otherwise — no schedule configured, no clone in 24 h, or a complete clone that posted no row — the wall clock in `tz` decides: inside `from`–`until` is unavailable, outside is healthy.  (Stephen's sentence names the clock, so the clock is specified; the narrower "only a genuine in-flight window" rule is the one open decision in the plan's §6.)

The `clusterhealth` row is `{"cluster":"corridor-dev1","bringup_exit":N,"status_exit":M,"at":"<ISO>"}`, POSTed by `cluster_restart_window.sh` on its own clone (pdy-dev work).  To make that possible, exec items now run with the same addressing a claude session gets: `ITER_WORKID`, `ITER_PROJECT`, `ITER_AGENT`, `ITER_TOPDIR`, `ITER_DATA_URL`, `ITER_ENGINE_TOKEN`, `ITER_BIN` and the `iter` shim on PATH (`work.rs` `run_exec`).

pdy-dev setup: set `cluster_restart.schedule` to the restart template id `9da12551-bb18-4adf-8fae-a4ca138fb317` in project settings (webui gear → cluster restart); until the window script posts the row, the clock rule alone holds.
...
**A declared block is not a bounce** (decided 2026-09-10, from pdy-dev item cb9c52c30bd6: four attempts and $59 re-measuring a block the record already declared).  `iter wait --on <id>… [--reason <text>]` links other items as deep dependencies of the CALLING item (one versioned PUT on `blockedby`, a `doc` row) — the verb for "I cannot finish this until X lands".  At close, when the verdict is incomplete and the record (re-read: the links were made during the run) names blockers that are still unsatisfied by the dependency rule (deep unless `blockedby_shallow`; `gate::waiting_on`), the engine writes a `verify` row with verdict `waiting` and the blocker ids, sets `lasterror: "close gate: waiting on N open blocker(s) […]"`, and puts the item back to `queued` with `gate_bounces` untouched and no question widget.  The dispatch dependency gate holds it there; when the blockers close complete it re-runs once with the open list as feedback.  Dispatch needed every blocker satisfied, so an open one at close is always a link the agent made this run.  An `unclear` verdict still goes to the human.
...
- **`iter` for agents**: the engine writes `{topdir}/.iter/bin/iter` (a shim to `iter_engine cli`) and puts it on PATH with ITER_BIN, ITER_PROJECT (name), ITER_WORKID, ITER_AGENT, ITER_TOPDIR, ITER_MAINFILE, ITER_CONTEXT_FILES, ITER_ITEM_CONTEXT_FILES, ITER_TEST_DIR, ITER_INTERFACE_DIR, ITER_USECASE_DIR, ITER_DATA_URL, ITER_ENGINE_TOKEN, BASH_MAX_TIMEOUT_MS.  Verbs: `add` (V2 flags and --file with V2 or V3 field names; child of the caller, requestedby `agent:<type>`, birth state from the agent's childstate, ids resolve by unique suffix, an item may not depend on its creator), `ask` (question widget + state question; the engine keeps that state at close and skips the gate), `reject` (parked + reason), `wait` (`--on <id>…`: deep dependencies on the calling item — the close gate queues an incomplete item behind them instead of bouncing it), `doc`, `critreview` (runs the critic persona, records a "review" row, `--disposition` closes the round — what the close gate's open-review check consumes), `capability`, `status`.  Local-file verbs (`runtests`, `validate`, `markers`, `teststate`, `usecase`) are served by V3 itself since 2026-09-04 (crate `iter3/iter_local`: the structureV2 marker scan, testgroups, the deterministic runner and validate, ported from V2 with the `.iter/config.iter.json` dependency replaced by topdir + main.iter.md) — **V3 has no dependency on the V2 binary or its files; both can be deleted from a project.**  The agent's command line is unchanged; from a plain shell inside a checkout the same verbs work with no engine or server.  `runtests` claims are recorded as "claim" detail rows on the calling item ({claim, group, upheld, outcome, counts}); a false `--broken` claim parks the item as stale (like `iter reject`), and the close gate holds an item whose LAST `--fixed` claim was false.  The V2-only verbs (`testsweep`, `orphans`, `resolve`) are retired and say so.
...
- **Session timeout** (fixed 2026-09-04): the project's per-agent `timeoutsec` override, else the agent record's `timeoutsec` (the Settings field — it was ignored before), else 3600.
...
- Testgroup layout, identical for every tested object (a usecase folder, a `repos/<container>/`, a component dir): `<object>/tests/iter/<name>/*.testgroup.iter.md`, `<object>/tests/iter/<name>/<env>/*.sh` (env ∈ dev|test|qa|prod|boot), shared programs in `{topdir}/.iter/tests/<env>/*` (the runner exports `ITER_TESTS_SHARED`). Discovery is recursive so the old `tests/iter/<env>.testgroup.iter.md` shape would still run; pdy-dev's 59 sets plus the usecase set WERE moved on 2026-09-08 (560 entries, ~1,100 files edited: links, prose paths, and every script's directory climbs gained one level), verified by running all 95 dev groups before and after: 391 of 396 tests identical, 3 improved, 1 fixed by hand. Tracked run transcripts moved to `<set>/evidence/`; `{topdir}/.iter/tests/README.md` documents the shared home. Lesson for the next such move: scripts locate the repo by climbing from their own directory, so a layout change is mostly a climb-depth change — count from the file's depth below the set, and watch for derived variables and cross-object references.
...
**Test run logs live on the work item, not in the tree** (`iter runtests`): no `runs/` directory; every run writes a `log_header` detail row (date, group, per-test pass/fail, no bodies); a non-green run also writes `log_detail` (failing scripts' output, 8 KB per script / 64 KB per row).  One pair per testgroup per ATTEMPT (decided 2026-09-10: an agent iterating on its tests had logged 16 runs / 31 rows on one item): a later run of the same group during the same attempt (header `ts` ≥ the item's `ts.start`) overwrites the previous run's header row and its adjacent detail row (`PUT …/details/{order}`; a red-then-green run replaces the old output with a one-line "green on the latest run" note) instead of appending; a new attempt starts a fresh pair. Project setting `fix_on_test_failure` (or a group's `auto_fix`): a non-green FULL run files a `code` item ("Tests non-green: testgroup …", codepath = the object above `tests/`, `createdby` = the running item so it inherits priority + usecase; tagged `check:tests-non-green` + `container:<testgroup label>` so the dedup key — not a title match — keeps one open fix item per group) — only when the runner is the Test Loop / an exec item / a human, never a code or testwriter session iterating on its own tests, never a `--broken` claim.
...
**Repeats are merged, never filed twice** (built 2026-09-10, `iter_core::dedup`, spec `iter3/plans/!iter_dedup_spec.md`; from the 2026-09-10 pdy-dev incident: one unchanged condition filed thirteen P1 items in ninety minutes). The identity of a repeat is two tags, `check:<rule label>` + `container:<name>`; both optional on the API, so no caller breaks. **Stage 1, iter_data, at create, no model:** a request carrying both tags while an OPEN item (not complete/failed/scheduled) carries the same two is not created — the open item comes back with `already_open: true` (HTTP 200) and is booked: a `doc` row "seen again <UTC> by <requestedby>" carrying the repeat's request text (the create body may carry `request`; it becomes detail row 0 on a normal create), `repeats` + 1 (new record field), priority halved rounding down with floor 1 (77 → 38 → 19 → 9 → 4 → 2 → 1; P0 stays), and the `repeated` tag once `repeats` reaches project setting `dedup.repeated_threshold` (default 3). A CLOSED twin does not block: the new item is created with a `doc` row "recurrence of <id>, which closed <ts>". `iter add` prints `already open: <id> …` and exits 0; the test-failure filer sends `check:tests-non-green` + `container:<label>` instead of matching titles. **Stage 2, engine, before the first dispatch, Sonnet:** each newly created queued item (attempt 0, not a schedule clone or exec, received after the feature landed, no `dedup_checked` stamp) is held one tick, tagged "blocked by: dedup triage", while a thread narrows deterministically — open items sharing its `container:`, its `check:`, or an overlapping lockdir; cap 20, newest first; none = no model call — and asks the engine-internal `dedup` agent (sonnet, Read/Glob/Grep, 180 s, run like `explain`, spend recorded on the item) for a one-line summary and exactly one of `same_fault` / `different` / `unsure` per candidate, as JSON. `same_fault` on one candidate: `POST …/workitems/{new}/duplicate_of {survivor, reason, others}` closes the newer item `complete` with the tag `dup of: <last 12 of the survivor id>` and a `doc` row "Duplicate of <id>: <reason>", and books the survivor as above; on several, the oldest open one survives and the rest are named on it. `unsure`: nothing merged, "may overlap with <id>: <reason>" `doc` rows on both. A survivor is never in-progress or closed and a duplicate is never in-progress (the endpoint refuses, the engine dispatches as usual). A judge that fails, times out or answers in prose stamps `dedup_checked` and the item dispatches as today. No `duplicate` state exists; the model is never in the create path.
